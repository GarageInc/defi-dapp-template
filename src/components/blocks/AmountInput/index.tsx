 ...
 * @constructor
 */
export const AmountInput = (props: IAmountInput) => {
  const { onMaxClicked, max, placeholder, rightTokenOptions, onChangeRightToken, maxDisabled, ...rest } = props
  const { value, onChange, leftToken, rightToken } = useAmountInput(props)
  ...
  const inputProps = useMemo(() => {
    ...
    if (leftToken) {
      result.startAdornment = (
        <InputAdornment position="start">
          <Symbol src={leftToken.icon} />
        </InputAdornment>
      )
    }
    if (onMaxClicked || rightToken) {
      result.endAdornment = (
        <InputAdornment position="end">
          {onMaxClicked && (
            <MaxButton onClick={onMaxClicked} disabled={maxDisabled}>
              Max
            </MaxButton>
          )}
          {rightToken ? (
    ...
    return result
  }, [leftToken, onMaxClicked, rightToken, maxDisabled, rightTokenOptions, onChangeRightToken])
...
const AmountInputWithMax = ({ inputValue, setInputValue, maxValue, decimals = 18, ...rest }: IAmountWithMax) => {
